...
contract DevmentorCore is DevmentorSessionManagement {
    ...
    ) DevmentorSessionManagement(_nativeToUsdpriceFeed) {}
    ...
    ) external view returns (DevmentorLib.Mentor memory) {
        return s_mentorByAccount[_mentorAddress];
    }

    function getStudent(
        address _studentAddress
    ) external view returns (DevmentorLib.Student memory) {
        return s_studentByAccount[_studentAddress];
    }

    function getSession(
        uint256 _sessionId
    ) external view returns (DevmentorLib.Session memory) {
    ...
    function getRoleByAccount(
        address _account
    ) external view returns (DevmentorLib.Role) {
